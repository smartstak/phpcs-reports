...
class WPSSLE_UsersFeedback {

	private $plugin_url         = WPSSLE_URL; // Changed - Better to use constant WPSSLE_URL for singel source of truth.
	private $plugin_version     = WPSSLE_VERSION; // Changed - Better to use constant WPSSLE_VERSION for singel source of truth.
	private $plugin_name        = 'WPSyncSheets Lite For Elementor';
	private $plugin_slug        = 'wpsse';
	private $plugin_download_id = WPSSLE_PLUGIN_ITEM_ID; // Changed - Better to use constant WPSSLE_VERSION for singel source of truth.
	/*
	|-----------------------------------------------------------------|
	...
	*/
	...
	/*
	|------------------------------------------------------------------|
	|   Enqueue all scripts and styles to required page only - Changed |
	|------------------------------------------------------------------|
	*/
				...
				<div class="<?php echo $this->plugin_slug; ?>-deactivation-response">
					<div id="<?php echo $this->plugin_slug; ?>-deactivate-feedback-dialog-header">
						<div class="<?php echo $this->plugin_slug; ?>-form-title--icon-wrapper">
							...
							<h2 class="<?php echo $this->plugin_slug; ?>-title"><?php echo esc_html( __( 'Quick Feedback', 'wpsse' ) ); ?></h2>
						</div>
					...
					<div id="<?php echo $this->plugin_slug; ?>-loader-wrapper">
						<div class="<?php echo $this->plugin_slug; ?>-loader-container">
								<img class="<?php echo $this->plugin_slug; ?>-preloader" src="<?php echo WPSSLE_URL; ?>feedback/images/preloader.gif">
						</div>
					</div>
					<div id="<?php echo $this->plugin_slug; ?>-form-wrapper" class="<?php echo $this->plugin_slug; ?>-form-wrapper-cls">
						<form id="<?php echo $this->plugin_slug; ?>-deactivate-feedback-dialog-form" method="post">
							...
							<div id="<?php echo $this->plugin_slug; ?>-deactivate-feedback-dialog-form-caption"><?php echo esc_html( __( 'If you have a moment, please share why you are deactivating this plugin.', 'wpsse' ) ); ?></div>
							<div id="<?php echo $this->plugin_slug; ?>-deactivate-feedback-dialog-form-body">
									...
									<div class="<?php echo $this->plugin_slug; ?>-deactivate-feedback-dialog-input-wrapper">
										...
										<?php if ( ! empty( $reason['input_placeholder'] ) ) : ?>
											...
											if ( in_array( $reason_key, $reason_key_arr, true ) ) {
												$twae_plugin_url = 'https://wordpress.org/plugins/wpsyncsheets-elementor/';
											}
										endif;
										?>
								...
								<?php endforeach; ?>
								<input class="<?php echo $this->plugin_slug; ?>-GDPR-data-notice" id="<?php echo $this->plugin_slug; ?>-GDPR-data-notice" type="checkbox"><label for="<?php echo $this->plugin_slug; ?>-GDPR-data-notice"><?php echo esc_html( __( 'I consent to WPSyncSheets plugins collecting all the information I submit through this form and using it to respond to my inquiry.', 'wpsse' ) ); ?></label>
							...
							<div class="<?php echo $this->plugin_slug; ?>-plugin-popup-button-wrapper">
								<a class="<?php echo $this->plugin_slug; ?>-button " id="<?php echo $this->plugin_slug; ?>-plugin-submitNdeactivate"><?php echo esc_html__( 'Submit and Deactivate', 'wpsse' ); ?></a>
								<a class="<?php echo $this->plugin_slug; ?>-button" id="<?php echo $this->plugin_slug; ?>-plugin-skipNdeactivate"><?php echo esc_html__( 'Skip and Deactivate', 'wpsse' ); ?></a>
							</div>
						</form>
					</div>
				</div>
	...
	public function get_deactivate_reasons_arr() {
		$deactivate_reasons = array(
			'didnt_work_as_expected'         => array(
				'title'             => esc_html( __( 'The plugin didn\'t work as expected', 'wpsse' ) ),
				'input_placeholder' => esc_html( __( 'What did you expect?', 'wpsse' ) ),
			),
			'found_a_better_plugin'          => array(
				'title'             => esc_html( __( 'I found a better plugin', 'wpsse' ) ),
				'input_placeholder' => esc_html( __( 'Please share which plugin', 'wpsse' ) ),
			),
			'couldnt_get_the_plugin_to_work' => array(
				'title'             => esc_html( __( 'The plugin is not working', 'wpsse' ) ),
				'input_placeholder' => esc_html( __( 'Please share your issue. So we can fix that for other users.', 'wpsse' ) ),
			),
			'temporary_deactivation'         => array(
				'title'             => esc_html( __( 'It\'s a temporary deactivation', 'wpsse' ) ),
				'input_placeholder' => '',
			),
			'other'                          => array(
				'title'             => esc_html( __( 'Other', 'wpsse' ) ),
				'input_placeholder' => esc_html( __( 'Please share the reason', 'wpsse' ) ),
			),
		);
		return $deactivate_reasons;
	}
}
new WPSSLE_UsersFeedback();