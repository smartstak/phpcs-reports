...
( function ( $ ) {
	...
	$( document ).ready( function () {
		$( '#authlink' ).on( 'click', function ( e ) {
			$( '#authbtn' ).hide();
			document.getElementById( 'authtext' ).style.display =
				'inline-block';
		} );
		...
		$( '#reset_settings' ).on( 'click', function ( e ) {
			e.preventDefault();
			const wpnonce = jQuery( '#wpssle_api_settings' ).val();
			jQuery.ajax( {
				url: admin_ajax_object.ajaxurl,
				type: 'post',
				data: 'action=wpssle_reset_settings&_wpnonce=' + wpnonce,
				beforeSend() {
					if (
						confirm( 'Are you sure you want to reset settings?' )
					) {
					} else {
						return false;
					}
				...
				success( response ) {
					if ( String( response ) === 'successful' ) {
						...
						window.location.href =
							admin_ajax_object.adminSettingsPageUrl;
					} else {
						alert( response );
					}
				},
...
jQuery( document ).ready( function ( $ ) {
	$( '.wpssle-nav-dashboard' ).addClass( 'active' );
	...
	$( '#wpsswinstallactivebtn' ).on( 'click', function () {
		const wpnonce = jQuery( '#wpssw_api_dashboard' ).val();
		const plugin_slug = 'wpsyncsheets-woocommerce'; // Replace with your plugin slug
		$( '#wpsswinstallactivebtn' ).html( 'Installing...' );
		jQuery.ajax( {
			url: admin_ajax_object.ajaxurl,
			type: 'post',
			data:
				'action=install_and_activate_plugin&plugin_slug=' +
				plugin_slug +
				'&_wpnonce=' +
				wpnonce,
			success( response ) {
				$( '#wpsswinstallactivebtn' ).html( 'Activated' );
				$( '#wpsswinstallactivebtn' ).toggleClass(
					'activated underline'
				);
	...
	$( '#wpsswactivebtn' ).on( 'click', function () {
		...
		jQuery.ajax( {
			...
			type: 'post',
			data:
				'action=install_and_activate_plugin&plugin_active=true&plugin_slug=' +
				plugin_slug +
				'&_wpnonce=' +
				wpnonce,
			success( response ) {
	...
	$( '#wpssginstallactivebtn' ).on( 'click', function () {
		const wpnonce = jQuery( '#wpssg_api_dashboard' ).val();
		const plugin_slug = 'wpsyncsheets-gravity-forms'; // Replace with your plugin slug
		...
		jQuery.ajax( {
			...
			success( response ) {
				...
				$( '#wpssginstallactivebtn' ).toggleClass(
					'activated underline'
				);
	...
	$( '#wpsscinstallactivebtn' ).on( 'click', function () {
		const wpnonce = jQuery( '#wpssc_api_dashboard' ).val();
		const plugin_slug = 'contactsheets-lite'; // Replace with your plugin slug
		...
		jQuery.ajax( {
			...
			success( response ) {
				...
				$( '#wpsscinstallactivebtn' ).toggleClass(
					'activated underline'
				);
	...
	$( '#wpsswpinstallactivebtn' ).on( 'click', function () {
		const wpnonce = jQuery( '#wpsswp_api_dashboard' ).val();
		const plugin_slug = 'wpsyncsheets-wpforms'; // Replace with your plugin slug
		...
		jQuery.ajax( {
			...
			success( response ) {
				...
				$( '#wpsswpinstallactivebtn' ).toggleClass(
					'activated underline'
				);
...
window.wpssleTab = function( evt, tabName ) {
...
window.wpssleNavTab = function( evt, tabName ) {
	'use strict';
	let i, tabContent, tabLinks;
	...
	const tablength = tabLinks.length;
...
window.wpssleNavTabLetsBegin = function( evt, tabName ) {
	'use strict';
	let i, tabContent, tabLinks;
	tabContent = document.getElementsByClassName( 'navtabcontent' );
	const tabContentlength = tabContent.length;
	...
	for ( i = 0; i < tablength; i++ ) {
		...
		if ( tabLinks[ i ].className == 'navtablinks wpssle-nav-googleapi' ) {
...
window.wpssleNavTabfreevspro = function( evt, tabName ) {
	'use strict';
	let i, tabContent, tabLinks;
	tabContent = document.getElementsByClassName( 'navtabcontent' );
	const tabContentlength = tabContent.length;
	...
	for ( i = 0; i < tablength; i++ ) {
		...
		if ( tabLinks[ i ].className == 'navtablinks wpssle-nav-freevspro' ) {
			tabLinks[ i ].className += ' active';
		}
	}
	document.getElementById( tabName ).style.display = 'block';
...
window.wpssleCopy = function( id, targetid ) {
