...
use WPSyncSheetsGravityForms\WPSSLGF_Google_API_Functions;
/**
 * Handle plugin installation upon activation.
 *
 * @since 1.0.0
 */
class WPSSLGF_Plugin_Settings {
	 ...
	 * @var $documentation
	 */
	protected static $documentation = 'https://docs.wpsyncsheets.com/wpssg-introduction/';
	/**
	 * Plugin support URL
	 *
	 * @var $submit_ticket
	 */
	protected static $submit_ticket = 'https://wordpress.org/support/plugin/wpsyncsheets-gravity-forms/';
	/**
	 * Plugin sheet setting URL
	 *
	 * @var $doc_sheet_setting
	 */
	protected static $doc_sheet_setting = 'https://docs.wpsyncsheets.com/wpssg-google-sheets-api-settings/';
	/**
	 * WPSyncSheets For Gravity Forms Pro Plugin URL
	 *
	 * @var $pro_version_url
	 */
	protected static $pro_version_url = 'https://www.wpsyncsheets.com/wpsyncsheets-for-gravity-forms/';
	/**
	 * Instance of WPSSLGF_Plugin_Settings
	 *
	 * @var $instance
	 */
	...
	public function __construct() {
		...
		add_action( 'admin_menu', array( $this, 'wpsslgf_menu_page' ), 30 );
		add_action( 'admin_enqueue_scripts', array( $this, 'wpsslgf_load_custom_wp_admin_style' ) );
		add_action( 'admin_enqueue_scripts', array( $this, 'wpsslgf_selectively_enqueue_admin_script' ) );
		add_filter( 'gform_noconflict_scripts', array( $this, 'wpsslgf_register_gform_noconflict_script' ) );
		add_filter( 'gform_noconflict_styles', array( $this, 'wpsslgf_register_gform_noconflict_style' ) );
		add_filter( 'plugin_row_meta', array( $this, 'wpsslgf_plugin_row_meta' ), 10, 2 );
		add_action( 'gform_loaded', array( $this, 'wpsslgf_load' ) );
		$this->wpsslgf_google_api();

		// Dashboard Install & Active AJAX
		add_action('wp_ajax_install_and_activate_plugin', array( $this, 'wpsslgf_install_and_activate_plugin') );
	}

	/**
	 * Main WPSSLGF_Plugin_Settings Instance.
	 *
	 * @since 1.0.0
	 *
	 * @return instance
	...
	/**
	 * Register GGSFeedAddOn Addon
	 */
	public function wpsslgf_load() {
		if ( ! method_exists( 'GFForms', 'include_feed_addon_framework' ) ) {
			return;
		}
		GFAddOn::register( 'WPSSLGF_Feed_Settings' );
	}

	/**
	 * Create WPSSLGF_Feed_Settings Class Instance.
	 */
	public static function get_addoninstance() {
		if ( null === self::$instanceaddon ) {
			self::$instanceaddon = new \WPSSLGF_Feed_Settings();
		}
	 ...
	 */
	public static function wpsslgf_google_api() {

		if ( null === self::$instance_api ) {
			self::$instance_api = new WPSSLGF_Google_API_Functions();
		}
	 ...
	 */
	public function wpsslgf_menu_page() {
		global $admin_page_hooks, $_parent_pages;
		if ( ! isset( $admin_page_hooks['wpsyncsheets_lite'] ) ) {
			$wpsslgf_page = add_menu_page(
				esc_attr__( 'WPSyncSheets Lite', 'wpssg' ),
				'WPSyncSheets Lite',
				'manage_options',
				'wpsyncsheets_lite',
				'',
				WPSSLGF_URL . 'assets/images/dashicons-wpsyncsheets.svg',
				90
			);
		}
		add_submenu_page( 'wpsyncsheets_lite', 'WPSyncSheets Lite For Gravity Forms', 'For Gravity Forms', 'manage_options', 'wpsyncsheets-gravity-forms', array( $this, 'wpsslgf_plugin_page' ), 3 );
	 ...
	 */
	public static function wpsslgf_load_custom_wp_admin_style() {

		$page = isset( $_GET['page'] ) ? sanitize_text_field( wp_unslash( $_GET['page'] ) ) : '';// phpcs:ignore

		$subview = isset( $_GET['subview'] ) ? sanitize_text_field( wp_unslash( $_GET['subview'] ) ) : '';// phpcs:ignore

		if ( $page && ( 'wpsyncsheets-gravity-forms' === $page ) ){ // phpcs:ignore
			wp_register_script(
				'wpsslgf-wp-admin',
				WPSSLGF_Assets_URL::assets_url(
					'js',
					'wpsslgf-admin-script',
					'assets/js/',
					'assets/js/build/'
				),
				array(),
			...
			wp_localize_script(
				...
				array(
					...
					'adminSettingsPageUrl' => admin_url( 'admin.php?page=wpsyncsheets-contact-form-7' ),
				)
			);
			wp_enqueue_script( 'wpsslgf-wp-admin' );
			if ( 'wpsyncsheets-gravity-forms' !== $page ) {
				wp_register_style(
					'wpsslgf-wp-admin',
					WPSSLGF_Assets_URL::assets_url(
						'css',
				...
				wp_enqueue_style( 'wpsslgf-wp-admin' );
			}
		}
		if ( $page && 'wpsyncsheets-gravity-forms' === $page ) {
			// phpcs:ignore
			wp_register_style(
				'wpsslgf-wp-admin-plugin-setting',
				WPSSLGF_Assets_URL::assets_url(
					'css',
					'wpsslgf-admin-plugin-setting-style',
					'assets/css/',
					'assets/css/build/'
				),
				false,
			...
			wp_enqueue_style( 'wpsslgf-wp-admin-plugin-setting' );

			wp_register_script( 'wpsslgf-bootstrap', 'https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js', array(), '3.4.1', true );
			wp_enqueue_script( 'wpsslgf-bootstrap' );
		}
		// phpcs:ignore
		if ( $page && ( 'gf_edit_forms' === $page ) && $subview && 'ggspreadsheetaddon' === $subview ) {
			wp_register_style(
				'wpsslgf-feed-pro',
				WPSSLGF_Assets_URL::assets_url(
					'css',
			...
			wp_enqueue_style( 'wpsslgf-feed-pro' );
		}
		wp_add_inline_style(
	        'wp-admin',
	 ...
	 */
	public static function wpsslgf_selectively_enqueue_admin_script() {
		wp_enqueue_script(
			'wpsslgf-general-script',
			WPSSLGF_Assets_URL::assets_url(
				'js',
				'wpsslgf-general',
				'assets/js/',
	...
	/**
	 * Register Gravity Forms no conflict Script
	 *
	 * @param array $scripts Enqueue Scripts.
	 */
	public static function wpsslgf_register_gform_noconflict_script( $scripts ) {
		$scripts[] = 'wpsslgf-wp-admin';
		$scripts[] = 'wpsslgf-general-script';
		return $scripts;
	}

	/**
	 * Register Gravity Forms no conflict Styles
	 *
	 * @param array $styles Enqueue Styles.
	 */
	public static function wpsslgf_register_gform_noconflict_style( $styles ) {
		$styles[] = 'wpsslgf-wp-admin';
		$styles[] = 'wpsslgf-feed-pro';
		return $styles;
	}
	 ...
	 *
	 * @param mixed $wpsslgf_links Plugin Row Meta.
	 * @param mixed $wpsslgf_file  Plugin Base file.
	 * @return  array
	 */
	public static function wpsslgf_plugin_row_meta( $wpsslgf_links, $wpsslgf_file ) {

		if ( 'wpsyncsheets-gravity-forms/wpsyncsheets-lite-gravity-forms.php' === (string) $wpsslgf_file ) {
			$wpsslgf_row_meta = array(
				'docs' => '<a href="' . esc_url( self::$documentation ) . '" title="' . esc_attr( __( 'View Documentation', 'wpssg' ) ) . '" target="_blank">' . __( 'View Documentation', 'wpssg' ) . '</a>',
			);
			return array_merge( $wpsslgf_links, $wpsslgf_row_meta );
		}
		return (array) $wpsslgf_links;
	}
	/**
	 * Show wpsslgf plugin screen.
	 */
	public function wpsslgf_plugin_page() {

		$wpsslgf_error           = '';
		$wpsslgf_token_error     = false;
		$wpsslgf_error_general   = '';
		$wpsslgf_apisettings     = '';
		$wpsslgf_generalsettings = '';
		$wpsslgf_emsettings      = '';
		$wpsslgf_supportsettings = '';
		if ( ! isset( $_GET['tab'] ) ) {
			// Google API Settings Tab.
			if ( isset( $_POST['submit'] ) || isset( $_POST['revoke'] ) ) {
				if ( ! isset( $_POST['wpsslgf_api_settings'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['wpsslgf_api_settings'] ) ), 'save_api_settings' ) ) {
					$wpsslgf_error = '<div class="error token_error"><p><strong class="err-msg">Error: Sorry, your nonce did not verify.</strong></p></div>';
				} else {
					if ( isset( $_POST['client_token'] ) ) {
						$wpsslgf_clienttoken = sanitize_text_field( wp_unslash( $_POST['client_token'] ) );
					} else {
						$wpsslgf_clienttoken = '';
					}
					if ( isset( $_POST['client_id'] ) && isset( $_POST['client_secret'] ) ) {
						$wpsslgf_google_settings = array( sanitize_text_field( wp_unslash( $_POST['client_id'] ) ), sanitize_text_field( wp_unslash( $_POST['client_secret'] ) ), $wpsslgf_clienttoken );
					} else {
						$wpsslgf_google_settings_value = self::$instance_api->wpsslgf_option( 'wpssg_google_settings' );
						$wpsslgf_google_settings       = array( $wpsslgf_google_settings_value[0], $wpsslgf_google_settings_value[1], $wpsslgf_clienttoken );
					}
					self::$instance_api->wpsslgf_update_option( 'wpssg_google_settings', $wpsslgf_google_settings );
					if ( isset( $_POST['revoke'] ) ) {
						$wpsslgf_google_settings    = self::$instance_api->wpsslgf_option( 'wpssg_google_settings' );
						$wpsslgf_google_settings[2] = '';
						self::$instance_api->wpsslgf_update_option( 'wpssg_google_settings', $wpsslgf_google_settings );
		...
		if ( isset( $_GET['code'] ) && ! empty( sanitize_text_field( wp_unslash( $_GET['code'] ) ) ) ) {
			$wpsslgf_code               = sanitize_text_field( wp_unslash( $_GET['code'] ) );
			$wpsslgf_token_value        = $wpsslgf_code;
			$wpsslgf_google_settings    = self::$instance_api->wpsslgf_option( 'wpssg_google_settings' );
			$wpsslgf_google_settings[2] = $wpsslgf_code;
			self::$instance_api->wpsslgf_update_option( 'wpssg_google_settings', $wpsslgf_google_settings );
		}

		$wpsslgf_google_settings_value = self::$instance_api->wpsslgf_option( 'wpssg_google_settings' );

		if ( ! empty( $wpsslgf_google_settings_value[2] ) ) {
			if ( ! self::$instance_api->checkcredenatials() ) {
				...
				if ( 'Invalid token format' === (string) $wpsslgf_error ) {
					$wpsslgf_error = '<div class="error token_error"><p><strong class="err-msg">Error: Invalid Token - Revoke Token with below settings and try again.</strong></p></div>';
				} else {
					$wpsslgf_error = '<div class="error token_error"><p><strong class="err-msg">Error: ' . $wpsslgf_error . '</strong></p></div>';
				}
				$wpsslgf_token_error = true;
			}
		}

		if ( empty( $wpsslgf_error ) && ! empty( $wpsslgf_google_settings_value[2] ) ) {
			try {
				...
				if ( 'insufficientPermissions' === (string) $reason ) {
					$wpsslgf_error = '<div class="error token_error"><p><strong class="err-msg">Error: Insufficient Permissions - Revoke Token with below settings and when generating access token select all the permissions.</strong></p></div>';
				}
				if ( 'accessNotConfigured' === (string) $reason ) {
					$wpsslgf_error = '<div class="error token_error"><p><strong class="err-msg">Error: Access not Configured - Please enable Google Sheets API and Google Drive API. Follow the <a target="_blank" href="https://docs.wpsyncsheets.com/wpssg-google-sheets-api-settings/" style="color:#000;">Google Sheets API Settings documentation</a> to enable APIs.</strong></p></div>';
				}
				if ( empty( $wpsslgf_error ) && '' !== (string) $reason ) {
					$wpsslgf_error = '<div class="error token_error"><p><strong class="err-msg">Error: Invalid Credentials - Reset settings and try again.</strong></p></div>';
				}
				$wpsslgf_token_error = true;
			}
		}
		$show_settings = false;
		if ( false === (bool) $wpsslgf_token_error ) {
			$show_settings = true;
		}
		?>
		<!-- .wrap -->
		<div class="wpsslgf-main-wrap">
			<div class="wpssw_order_top_header"> <!-- ONLY this will be hidden -->
						<div class="wpssw_top_header_content_wrapper">
							<p class="wpssw_top_header_text">
								...
								<a href="<?php echo WPSSLGF_BUY_PRO_VERSION_URL;  ?>" target="_blank">upgrade to pro →</a>
							</p>
						</div>
						<button class="wpssw_close_btn" onclick="wc_closeTopHeader()">×</button>
			</div>
			<div class="wpsslgf-header-main">
				<div class="container">
					<div class="wpsslgf-header-section">
						<div class="wpsslgf-header-left">
							<div class="wpsslgf-logo-section">
								<img src="<?php echo esc_url( WPSSLGF_URL . 'assets/images/logo.svg?ver=' ); ?><?php echo esc_attr( WPSSLGF_VERSION ); ?>">
							</div>
							<div class="wpsslgf-nav-top">
								<ul>
									<li>
										<button class="navtablinks wpsslgf-nav-dashboard" onclick="wpsslgfNavTab(event, 'wpsslgf-nav-dashboard')">
									...
									<li>
										<button class="navtablinks wpsslgf-nav-googleapi" onclick="wpsslgfNavTab(event, 'wpsslgf-nav-googleapi')">
									...
									<li>
										<button class="navtablinks wpsslgf-nav-freevspro" onclick="wpsslgfNavTab(event, 'wpsslgf-nav-freevspro')">
						...
						<div class="wpsslgf-header-right">
							<ul class="wpsslgf-header-links">
								<li class="version"><span>V<?php echo esc_html( WPSSLGF_VERSION ); ?></span></li>
								<li id="wpsslgfBtn">
									<a target="_blank" href="https://docs.wpsyncsheets.com/wpssg-introduction/">
							...
							<div class="whatsNew-block">
								...
								<div class="wpsslgf-block-content changeLog-conntent">
									<?php
									$changelog = self::wpsslgf_get_plugin_changelog( WP_PLUGIN_DIR . '/wpsyncsheets-gravity-forms' );
									// phpcs:ignore
			...
			<!-- messages html end-->		
			<div class="wpsslgf-tabs-main">

			    <!-- DashBoard html start   -->
				<div id="wpsslgf-nav-dashboard" class="navtabcontent vertical-tabs">
							...
							<div class="wpss-dashboard-navtabcontent">
								...
								<div class="welcome-wpsyncsheets-section">
									<div class="welcome-data">
										...
										<p>
											WPSyncSheets For Gravity Forms is designed to simplify your workflow by syncing your website forms data directly with Google Sheets. To explore all its features, check out the official documentation and video tutorials.
										</p>
									</div>
									<div class="unlock-pro-button-section">
										<a class="underline" href="javascript:void(0)" onclick="wpsslgfNavTabLetsBegin(event, 'wpsslgf-nav-googleapi')">
											Let's Connect
										</a>
									</div>
								</div>
							...
							<div class="wpss-setup-guide-navtabcontent">
								...
								<div class="setup-guide-section">
									<div class="setup-guide-data">
										...
										<p>
											Sync Gravity Forms data with Google Sheets in real-time — effortlessly and accurately.
										</p>
									</div>
									<div class="setup-content-data">
										<div class="setup-row">
											<div class="google-api-setting-guide">
												...
												<ul>
													<li><a href="https://docs.wpsyncsheets.com/wpssg-google-sheets-api-settings/" target="_blank">Google API Settings Guide</a></li>
													...
													<li><a href="https://docs.wpsyncsheets.com/wpssg-faq/#appverified" target="_blank">How to resolve "This app isn’t verified"?</a></li>
													<li><a href="https://docs.wpsyncsheets.com/wpssg-faq/#rangeexceed" target="_blank">How to resolve range exceeds grid limits?</a></li>
												</ul>
											</div>
											<div class="google-api-setting-guide">
												<h1>Miscellaneous</h1>
												<ul>
													<li><a href="https://docs.wpsyncsheets.com/wpssg-plugins-compatibility/">Plugins Compatibility</a></li>
													<li><a href="https://docs.wpsyncsheets.com/wpssg-update/">Update Plugin</a></li>
												</ul>
											</div>
										</div>
										...
										<div class="setup-row">
											<div class="google-api-setting-guide">
												<h1>Export</h1>
												<ul>
													<li><a href="https://docs.wpsyncsheets.com/how-to-export-gf-entries/" target="_blank">How to Export Entries?</a></li>
												</ul>
											</div>
											<div class="google-api-setting-guide">
												<h1>Import<span>Pro</span></h1>
												<ul>
													<li><a href="https://docs.wpsyncsheets.com/how-to-import-gf-entries/" target="_blank">How to Import Entries?</a></li>
												</ul>
											</div>
										</div>
									</div>
							...
							<div class="wpss-dashboard-related-plugins">
								<div class="wpss-import-export-plugins">
									...
									<div class="import-export-plugin-section">
										...
										<div class="import-export-plugin-info">
											<div class="import-export-feature-img">
												<img src="<?php echo esc_url( WPSSLGF_URL . 'assets/images/woocommerce-icon.svg' ); ?>">
											</div>
											<div class="import-export-box-border">
												...
												<div class="import-export-button-section">
													...
													$wpssw_status = WPSSLGF_Dependencies::check_plugin_install_and_activation_status($wpssw_plugin_slug, $wpssw_plugin_file);
													?>
										...
										<div class="import-export-plugin-info">
											<div class="import-export-box-border">
												...
												<div class="import-export-button-section">
													...
													$wpsse_status = WPSSLGF_Dependencies::check_plugin_install_and_activation_status($wpsse_plugin_slug, $wpsse_plugin_file);
													?>
											...
											<div class="import-export-feature-img">
												<img src="<?php echo esc_url( WPSSLGF_URL . 'assets/images/Elementor-icon.svg' ); ?>">
											</div>
										</div>
									</div>
									...
									<div class="import-export-plugin-section">
										...
										<div class="import-export-plugin-info">
											<div class="import-export-feature-img">
												<img src="<?php echo esc_url( WPSSLGF_URL . 'assets/images/ContactForm-icon.svg' ); ?>">
											</div>
											<div class="import-export-box-border">
												<div class="import-export-pro-content">
												...
												<div class="import-export-button-section">
													...
													$wpssc_status = WPSSLGF_Dependencies::check_plugin_install_and_activation_status($wpssc_plugin_slug, $wpssc_plugin_file);
													?>
										...
										<div class="import-export-plugin-info">
											<div class="import-export-box-border">
												...
												<div class="import-export-button-section">
													...
													$wpsswp_status = WPSSLGF_Dependencies::check_plugin_install_and_activation_status($wpsswp_plugin_slug, $wpsswp_plugin_file);
													?>
													...
													<?php } else if(! $wpsswp_status['installed']) { ?>
											...
											<div class="import-export-feature-img">
												<img src="<?php echo esc_url( WPSSLGF_URL . 'assets/images/WPForms-icon.svg' ); ?>">
											</div>
										</div>
									</div>
								...
								<div class="video-grid">
									<div class="video-item">
										<a href="#" target="_blank" class="play-icon" data-video-id="Z6-wzgpRlLE">
										    <img src="<?php echo esc_url( WPSSLGF_URL . 'assets/images/WPSyncSheets-Intro.jpg' ); ?>" alt="">
										</a>
									</div>
									<div class="video-item">
										<a href="#" target="_blank" class="play-icon" data-video-id="EFLcBu-Jgp8">
										    <img src="<?php echo esc_url( WPSSLGF_URL . 'assets/images/client-id-client-secret.jpg' ); ?>" alt="">
										</a>
									</div>
								</div>
								<div id="video-popup">
				...
				<div id="wpsslgf-nav-googleapi" class="navtabcontent vertical-tabs">
					<div id="googleapi-settings" class="wpsslgf-navtabcontent">
						<?php
						if ( ! current_user_can( 'edit_wpsyncsheets_gravity_forms_lite_main_settings' ) ) {
							?>
						<div class="wpsslgf-top-subtext generalSetting-section">
							<div class="generalSetting-left">
								<h4><?php echo esc_html__( 'You do not have permission to access this page.', 'wpssg' ); ?></h4>
							</div>
						</div>
							<?php
						} else {
						...
						<div class="wpsslgf-top-subtext generalSetting-section">
							<div class="generalSetting-left">
								<h4><?php echo esc_html__( 'Google API Settings', 'wpssg' ); ?></h4>
								<p><?php echo esc_html__( 'Google APIs allow you to embed Google Services in your online site. To connect Google Drive and Google Sheets with your WordPress website, you need to generate dedicated API keys. To begin the process, kindly log in to your Gmail Account and follow the link to initiate the setup ', 'wpssg' ); ?><a href="<?php echo esc_url( self::$doc_sheet_setting ); ?>" target="_blank"><?php echo esc_html__( 'click here', 'wpssg' ); ?>.</a></p>
							</div>
						</div>
						<form method="post" action="<?php echo esc_html( admin_url( 'admin.php?page=wpsyncsheets-gravity-forms' ) ); ?>">
							<?php wp_nonce_field( 'save_api_settings', 'wpsslgf_api_settings' ); ?>
							<div id="universal-message-container">
								<div class="generalSetting-section">
									<div class="integrationtabform">
										<div class="options">
											<ul class="form-table">
												<li>
													<label for="client_id"> <?php echo esc_html__( 'Client ID', 'wpssg' ); ?> </label>
													<div class="forminp forminp-text">
														<input type="text" id="client_id" required name="client_id" value="<?php echo isset( $wpsslgf_google_settings_value[0] ) ? esc_attr( $wpsslgf_google_settings_value[0] ) : ''; ?>" size="80" class = "googlesettinginput" placeholder="Enter Client Id" 
															<?php
															if ( ! empty( $wpsslgf_google_settings_value[0] ) ) {
																echo 'readonly';
															}
															?>
														/>
												...
												<li>
													<label for="client_secret"> <?php echo esc_html__( 'Client Secret Key', 'wpssg' ); ?> </label>
													<div class="forminp forminp-text">
														<input type="text" id="client_secret" required name="client_secret" value="<?php echo isset( $wpsslgf_google_settings_value[1] ) ? esc_attr( $wpsslgf_google_settings_value[1] ) : ''; ?>" size="80" class = "googlesettinginput" placeholder="Enter Client Secret" 
															<?php
															if ( ! empty( $wpsslgf_google_settings_value[1] ) ) {
																echo 'readonly';
															}
															?>
														/>
												...
												<?php
												if ( ! empty( $wpsslgf_google_settings_value[0] ) && ! empty( $wpsslgf_google_settings_value[1] ) ) {
														$wpsslgf_token_value = $wpsslgf_google_settings_value[2];
													?>
												<li>
													<label><?php echo esc_html__( 'Client Token', 'wpssg' ); ?></label>
													<?php
													if ( empty( $wpsslgf_token_value ) && ! isset( $_GET['code'] ) ) {
														$wpsslgf_auth_url = self::$instance_api->getClient();
														self::$instance_api->wpsslgf_update_option( 'wpssg_share_enable', true );
														?>
														<div id="authbtn">
															<a href="<?php echo esc_url( $wpsslgf_auth_url ); ?>" id="authlink" target="_blank" ><div class="wpsslgf-button wpsslgf-button-secondary"><?php echo esc_html__( 'Click here to generate an Authentication Token', 'wpssg' ); ?></div></a>
														</div>
														<?php
													}
													$wpsslgf_code                  = '';
													...
													<div  id="authtext" 
													<?php
													if ( ! empty( $wpsslgf_token_value ) || $wpsslgf_code ) {
														echo 'class = "forminp forminp-text wpsslgf-authtext" ';
													} else {
														echo 'class="forminp forminp-text"'; }
													?>
														><input type="text" name="client_token" value="<?php echo $wpsslgf_token_value ? esc_attr( $wpsslgf_token_value ) : esc_attr( $wpsslgf_code ); ?>" size="80" placeholder="Please enter authentication code" id="client_token" class="googlesettinginput" 
															<?php
															if ( ! empty( $wpsslgf_google_settings_value[2] ) ) {
																echo 'readonly';
															}
												...
												<?php } if ( ! empty( $wpsslgf_token_value ) ) { ?>
												<li>
													<label></label>
													<div><input type="submit" name="revoke" id="revoke" value = "Revoke Token" class="wpsslgf-button wpsslgf-button-secondary"/></div>
												</li>
												<?php } ?>
											</ul>
										</div>
										...
										<div class="submit-section">
											<p class="submit">
												<input type="submit" name="submit" id="submit" class="wpsslgf-button wpsslgf-button-primary" value="Save">
												<?php
												if ( ! empty( $wpsslgf_token_value ) || ! empty( $wpsslgf_google_settings_value[0] ) || ! empty( $wpsslgf_google_settings_value[1] ) ) {
													?>
														<input type="submit" name="reset_settings" id="reset_settings" value = "Reset" class="wpsslgf-button wpsslgf-button-primary reset_settings"/>
													<?php } ?>
											</p>
										</div>
									</div>
								...
								<div class="generalSetting-section copy-url-table">
									<ul>
										<li>
											<label><?php echo esc_html__( 'Authorized Domain : ', 'wpssg' ); ?></label>
											<div class="copy-url-text"><span id="authorized_domain"><?php echo esc_html( $site_url ); ?></span>
												<span class="copy-icon wpsslgf-button" id="a_domain" onclick="wpsslgfCopy('authorized_domain','a_domain');">
										...
										<li>
											<label><?php echo esc_html__( 'Authorised redirect URIs : ', 'wpssg' ); ?></label>
											<div class="copy-url-text">
												<span id="authorized_uri"><?php echo esc_html( admin_url( 'admin.php?page=wpsyncsheets-gravity-forms' ) ); ?></span>
												<span class="copy-icon wpsslgf-button tooltip-click1" onclick="wpsslgfCopy('authorized_uri','a_uri');" id="a_uri">
													<svg width="15" height="18" viewBox="0 0 15 18" fill="none" xmlns="http://www.w3.org/2000/svg">
													<path d="M2.16667 17.3334C1.70833 17.3334 1.31597 17.1702 0.989583 16.8438C0.663194 16.5174 0.5 16.1251 0.5 15.6667V4.00008H2.16667V15.6667H11.3333V17.3334H2.16667ZM5.5 14.0001C5.04167 14.0001 4.64931 13.8369 4.32292 13.5105C3.99653 13.1841 3.83333 12.7917 3.83333 12.3334V2.33341C3.83333 1.87508 3.99653 1.48272 4.32292 1.15633C4.64931 0.829942 5.04167 0.666748 5.5 0.666748H13C13.4583 0.666748 13.8507 0.829942 14.1771 1.15633C14.5035 1.48272 14.6667 1.87508 14.6667 2.33341V12.3334C14.6667 12.7917 14.5035 13.1841 14.1771 13.5105C13.8507 13.8369 13.4583 14.0001 13 14.0001H5.5ZM5.5 12.3334H13V2.33341H5.5V12.3334Z" fill="#383E46"/>
													</svg>
													<span class="tooltip-text">Copied</span>
				...
				<div id="wpsslgf-nav-freevspro" class="navtabcontent vertical-tabs">
					<div class="wpsslgf-navtabcontent">
							<!-- Header Section -->
							<div class="free-pro-header">
								...
								<div class="unlock-pro-button-section">
									<a class="underline" href="<?php echo WPSSLGF_BUY_PRO_VERSION_URL;  ?>" target="_blank" rel="noreferrer">
										Upgrade Now
									</a>
								</div>
							</div>
							...
							<div class="pro-free-features">
									...
									<!-- Feature: Support multiple forms to sheets -->
									<div class="feature-data">
										<p>Support Multiple Forms to Sheets</p>
											<div class="feature-icon">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#16A34A" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check">
												<path d="M20 6 9 17l-5-5"></path>
												</svg>
									...
									<!-- Feature: Enable/Disable Form ID & Form Title -->
									<div class="feature-data">
										<p>Enable/Disable Form ID & Form Title</p>
											<div class="feature-icon">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#16A34A" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check">
												<path d="M20 6 9 17l-5-5"></path>
												</svg>
									...
									<!-- Feature: Export All Entries -->
									<div class="feature-data">
										<p>Export All Entries</p>
											<div class="feature-icon">
									...
									<!-- Feature: Export Entries with Date Range -->
									<div class="feature-data">
										<p>Export Entries with Date Range</p>
											<div class="feature-icon">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#DC2626" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-x">
												<path d="M18 6 6 18"></path>
												<path d="m6 6 12 12"></path>
									...
									<!-- Feature: Schedule Auto Sync Entries -->
									<div class="feature-data">
										<p>Schedule Auto Sync Entries</p>
											<div class="feature-icon">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#DC2626" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-x">
												<path d="M18 6 6 18"></path>
												<path d="m6 6 12 12"></path>
									...
									<!-- Feature: Update Rows when Entries are Updated -->
									<div class="feature-data">
										<p>Update Rows when Entries are Updated</p>
											<div class="feature-icon">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#DC2626" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-x">
												<path d="M18 6 6 18"></path>
												<path d="m6 6 12 12"></path>
									...
									<!-- Feature: Upload Files to Google Drive on Submission -->
									<div class="feature-data">
										<p>Upload Files to Google Drive on Submission</p>
											<div class="feature-icon">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#DC2626" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-x">
												<path d="M18 6 6 18"></path>
												<path d="m6 6 12 12"></path>
									...
									<!-- Feature: Enable Conditional Logic -->
									<div class="feature-data">
										<p>Enable Conditional Logic</p>
											<div class="feature-icon">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#DC2626" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-x">
												<path d="M18 6 6 18"></path>
												<path d="m6 6 12 12"></path>
				...
				if ( ! empty( $wpsslgf_error ) || ! empty( $wpsslgf_error_general ) || ! \WPSSLGF_Dependencies::wpsslgf_is_gravityform_plugin_active() ) {
					$error_msg = '';
					if ( ! \WPSSLGF_Dependencies::wpsslgf_is_gravityform_plugin_active() ) {
						$error_msg = 'WPSyncSheets Lite For Gravity Forms plugin requires <a href="' . esc_url( 'https://www.gravityforms.com/' ) . '" target= "_blank">Gravity Forms</a> plugin to be active!';
						?>
					<input type="hidden" id="error-message" value="<?php echo esc_attr( $error_msg ); ?>">
						<?php
					} else {
						if ( ! empty( $wpsslgf_error_general ) ) {
							$error_msg = wp_strip_all_tags( $wpsslgf_error_general );
						} else {
							$error_msg = wp_strip_all_tags( $wpsslgf_error );
						}
						if ( $error_msg ) {
							?>
						<input type="hidden" id="error-message" value="<?php echo esc_attr( $error_msg ); ?>">
				...
				<script type="text/javascript">
					jQuery.noConflict();
					jQuery(".wpsslgf-header-section .whatsNew-toggle, .wpsslgf-header-section .close-block").click(function(){
						jQuery(".wpsslgf-header-section .wpsslgf-header-right").toggleClass('active');
						jQuery(".wpsslgf-header-section .whatsNew-toggle").toggleClass('active');
					});
				</script>
                <div class="clear"></div>
			</div>
            ...
            <!-- Unlock Pro Features -->
			<div class="wpsslgf-tabs-main wpsslgf-pro-main">
						...
						<div class="generalSetting-section">
							<div class="box-border">
								<div class="unlock-pro-content">
									...
									<h1>Seamless Import & Export — No Limits, No Hassle!</h1>
								</div>
									<p class="text">Sync Gravity Forms data with Google Sheets in real-time — effortlessly and accurately.</p>
                                ...
                                <ul class="pro-features-content">
									<li>
										...
										<path d="M20 6 9 17l-5-5"></path></svg><span>Export All Entries</span>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check text-brand-primary-600">
										<path d="M20 6 9 17l-5-5"></path></svg><span>Import Entries</span>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check text-brand-primary-600">
										<path d="M20 6 9 17l-5-5"></path></svg><span>Schedule Auto Sync Entries</span>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check text-brand-primary-600">
										<path d="M20 6 9 17l-5-5"></path></svg><span>Upload Files to Google Drive on Submission</span>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check text-brand-primary-600">
										<path d="M20 6 9 17l-5-5"></path></svg><span>Update Rows when Entries are Updated</span>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check text-brand-primary-600">
								...
								<div class="unlock-pro-button-section">
									...
									<a class="no-underline" href="javascript:void(0)" onclick="wpsslgfNavTabfreevspro(event, 'wpsslgf-nav-freevspro')">
									    Free vs Pro
								    </a>
								</div>
							</div>
							...
							<div class="pro-feature-img">
								<img src="<?php echo esc_url( WPSSLGF_URL . 'assets/images/gform-pro.png' ); ?>" />
							</div>
						</div>	
					</div>
				</div>
	 ...
	 */
	public static function wpsslgf_get_plugin_changelog( $plugin_path ) {
	...
	*/
	public static function wpsslgf_install_and_activate_plugin() {
