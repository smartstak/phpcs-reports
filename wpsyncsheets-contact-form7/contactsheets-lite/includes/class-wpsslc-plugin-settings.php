...
class WPSSLC_Plugin_Settings {
	 ...
	 */
	protected static $doc_sheet_setting = 'https://docs.wpsyncsheets.com/wpssc-google-sheets-api-settings/';
	/**
	 * Url for plugin support.
	 *
	 * @var $submit_ticket
	 */
	protected static $submit_ticket = 'https://wordpress.org/support/plugin/contactsheets-lite/';
	/**
	 * Instance of class.
	 *
	...
	public function __construct() {
		...
		add_action( 'admin_menu', array( $this, 'wpsslc_menu_page' ), 40 );
		...
		add_action('wp_ajax_install_and_activate_plugin', array( $this, 'wpsslc_install_and_activate_plugin') );
	}
	/**
	...
	public function wpsslc_menu_page() {
		...
		if ( ! isset( $admin_page_hooks['wpsyncsheets_lite'] ) ) {
			$wpsslc_page = add_menu_page(
				...
				'wpsyncsheets_lite',
				'',
				WPSSLC_URL . 'assets/images/dashicons-wpsyncsheets.svg',
				90
			);
		}
		add_submenu_page( 'wpsyncsheets_lite', 'WPSyncSheets Lite For Contact Form 7', 'For Contact Form 7', 'manage_options', 'wpsyncsheets-contact-form-7', array( $this, 'wpsslc_plugin_page' ), 4 );
	...
	public static function wpsslc_load_custom_wp_admin_style() {
		...
		if ( is_admin() && ( $page && ( ( 'wpcf7' === $page ) || ( 'wpsyncsheets-contact-form-7' === $page ) || ( 'wpcf7-new' === $page ) ) ) ) {
		// @codingStandardsIgnoreEnd.
			wp_enqueue_script(
				'wpsslc-admin-setting',
			...
			if ( 'wpsyncsheets-contact-form-7' !== $page ) {
				wp_enqueue_style(
					'wpsslc-admin-setting',
					WPSSLC_Assets_URL::assets_url(
						'css',
		...
		if ( $page && 'wpsyncsheets-contact-form-7' === $page ) {
			// phpcs:ignore
			wp_register_style(
				'wpsslc-wp-admin-plugin-setting',
				WPSSLC_Assets_URL::assets_url(
					'css',
					'wpsslc-admin-plugin-setting-style',
					'assets/css/',
					'assets/css/build/'
				),
				false,
			...
			wp_enqueue_style( 'wpsslc-wp-admin-plugin-setting' );

			wp_register_script( 'wpsslc-bootstrap', 'https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js', array(), '3.4.1', true );
			wp_enqueue_script( 'wpsslc-bootstrap' );
		}
		wp_add_inline_style(
	        'wp-admin',
	...
	public function wpsslc_selectively_enqueue_admin_script() {
		wp_enqueue_script(
			...
			WPSSLC_URL . 'assets/js/wpsslc-general.js',
			WPSSLC_VERSION,
			true,
			false
		);
	...
	/**
	 * Show wpsslc plugin screen.
	 */
	public function wpsslc_plugin_page() {

		$wpsslc_error           = '';
		$wpsslc_token_error     = false;
		$wpsslc_error_general   = '';
		$wpsslc_apisettings     = '';
		$wpsslc_generalsettings = '';
		$wpsslc_emsettings      = '';
		$wpsslc_supportsettings = '';
		if ( ! isset( $_GET['tab'] ) ) {
			...
			if ( isset( $_POST['submit'] ) || isset( $_POST['revoke'] ) ) {
				if ( ! isset( $_POST['wpsslc_api_settings'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['wpsslc_api_settings'] ) ), 'save_api_settings' ) ) {
					$wpsslc_error = '<div class="error token_error"><p><strong class="err-msg">Error: Sorry, your nonce did not verify.</strong></p></div>';
				} else {
		...
		if ( isset( $_GET['code'] ) && ! empty( sanitize_text_field( wp_unslash( $_GET['code'] ) ) ) ) {
			...
			$wpsslc_token_value        = $wpsslc_code;
			$wpsslc_google_settings    = self::$instance_api->wpsslc_option( 'wpssc_google_settings' );
			$wpsslc_google_settings[2] = $wpsslc_code;
			self::$instance_api->wpsslc_update_option( 'wpssc_google_settings', $wpsslc_google_settings );
		}
		...
		if ( ! empty( $wpsslc_google_settings_value[2] ) ) {
			if ( ! self::$instance_api->checkcredenatials() ) {
				...
				if ( 'Invalid token format' === (string) $wpsslc_error ) {
					$wpsslc_error = '<div class="error token_error"><p><strong class="err-msg">Error: Invalid Token - Revoke Token with below settings and try again.</strong></p></div>';
				} else {
					$wpsslc_error = '<div class="error token_error"><p><strong class="err-msg">Error: ' . $wpsslc_error . '</strong></p></div>';
				}
				$wpsslc_token_error = true;
			}
		}

		if ( empty( $wpsslc_error ) && ! empty( $wpsslc_google_settings_value[2] ) ) {
			try {
				...
				if ( 'insufficientPermissions' === (string) $reason ) {
					$wpsslc_error = '<div class="error token_error"><p><strong class="err-msg">Error: Insufficient Permissions - Revoke Token with below settings and when generating access token select all the permissions.</strong></p></div>';
				}
				if ( 'accessNotConfigured' === (string) $reason ) {
					$wpsslc_error = '<div class="error token_error"><p><strong class="err-msg">Error: Access not Configured - Please enable Google Sheets API and Google Drive API. Follow the <a target="_blank" href="https://docs.wpsyncsheets.com/wpssc-google-sheets-api-settings/" style="color:#000;">Google Sheets API Settings documentation</a> to enable APIs.</strong></p></div>';
				}
				if ( empty( $wpsslc_error ) && '' !== (string) $reason ) {
					$wpsslc_error = '<div class="error token_error"><p><strong class="err-msg">Error: Invalid Credentials - Reset settings and try again.</strong></p></div>';
				}
				$wpsslc_token_error = true;
			}
		}
		$show_settings = false;
		if ( false === (bool) $wpsslc_token_error ) {
			$show_settings = true;
		}
		?>
		<!-- .wrap -->
		<div class="wpsslc-main-wrap">
			<div class="wpssw_order_top_header"> <!-- ONLY this will be hidden -->
						<div class="wpssw_top_header_content_wrapper">
							<p class="wpssw_top_header_text">
								...
								<a href="<?php echo WPSSLC_PRO_VERSION_BUY_URL;  ?>" target="_blank">upgrade to pro →</a>
							</p>
						</div>
						<button class="wpssw_close_btn" onclick="wc_closeTopHeader()">×</button>
			</div>
			<div class="wpsslc-header-main">
				<div class="container">
					<div class="wpsslc-header-section">
						<div class="wpsslc-header-left">
							<div class="wpsslc-logo-section">
								<img src="<?php echo esc_url( WPSSLC_URL . 'assets/images/logo.svg?ver=' ); ?><?php echo esc_attr( WPSSLC_VERSION ); ?>">
							</div>
							<div class="wpsslc-nav-top">
								<ul>
									<li>
										<button class="navtablinks wpsslc-nav-dashboard" onclick="wpsslcNavTab(event, 'wpsslc-nav-dashboard')">
									...
									<li>
										<button class="navtablinks wpsslc-nav-googleapi" onclick="wpsslcNavTab(event, 'wpsslc-nav-googleapi')">
									...
									<li>
										<button class="navtablinks wpsslc-nav-freevspro" onclick="wpsslcNavTab(event, 'wpsslc-nav-freevspro')">
						...
						<div class="wpsslc-header-right">
							<ul class="wpsslc-header-links">
								<li class="version"><span>V<?php echo esc_html( WPSSLC_VERSION ); ?></span></li>
								<li id="wpsslcBtn">
									<a target="_blank" href="<?php echo WPSSLC_DOCS_LINK; ?>">
							...
							<div class="whatsNew-block">
								...
								<div class="wpsslc-block-content changeLog-conntent">
									<?php
									$changelog = self::wpsslc_get_plugin_changelog( WP_PLUGIN_DIR . '/contactsheets-lite' );
									// phpcs:ignore
			...
			<!-- messages html end-->		
			<div class="wpsslc-tabs-main">

			     <!-- DashBoard html start   -->
				<div id="wpsslc-nav-dashboard" class="navtabcontent vertical-tabs">
							...
							<div class="wpss-dashboard-navtabcontent">
								...
								<div class="welcome-wpsyncsheets-section">
									<div class="welcome-data">
										...
										<p>
											WPSyncSheets For Contact Form 7 is designed to simplify your workflow by syncing your website forms data directly with Google Sheets. To explore all its features, check out the official documentation and video tutorials.
										</p>
									</div>
									<div class="unlock-pro-button-section">
										<a class="underline" href="javascript:void(0)" onclick="wpsslcNavTabLetsBegin(event, 'wpsslc-nav-googleapi')">
											Let's Connect
										</a>
									</div>
								</div>
							...
							<div class="wpss-setup-guide-navtabcontent">
								...
								<div class="setup-guide-section">
									<div class="setup-guide-data">
										...
										<p>
											Sync Contact Form 7 data with Google Sheets in real-time — effortlessly and accurately.
										</p>
									</div>
									<div class="setup-content-data">
										<div class="setup-row">
											<div class="google-api-setting-guide">
												...
												<ul>
													<li><a href="https://docs.wpsyncsheets.com/wpssc-google-sheets-api-settings/" target="_blank">Google API Settings Guide</a></li>
													...
													<li><a href="https://docs.wpsyncsheets.com/wpssc-faq/#appverified" target="_blank">How to resolve "This app isn’t verified"?</a></li>
													<li><a href="https://docs.wpsyncsheets.com/wpssc-faq/#rangeexceed" target="_blank">How to resolve range exceeds grid limits?</a></li>
												</ul>
											</div>
											<div class="google-api-setting-guide">
												<h1>Miscellaneous</h1>
												<ul>
													<li><a href="https://docs.wpsyncsheets.com/wpssc-filters-hooks-snippets/">Filters, Hooks & Snippets</a></li>
												</ul>
											</div>
										</div>
										...
										<div class="setup-row">
											<div class="google-api-setting-guide">
												<h1>Export</h1>
												<ul>
													<li><a href="https://docs.wpsyncsheets.com/how-to-export-cf-entries/" target="_blank">How to Export Entries?</a></li>
												</ul>
											</div>
										</div>
									</div>
							...
							<div class="wpss-dashboard-related-plugins">
								<div class="wpss-import-export-plugins">
									...
									<div class="import-export-plugin-section">
										...
										<div class="import-export-plugin-info">
											<div class="import-export-feature-img">
												<img src="<?php echo esc_url( WPSSLC_URL . 'assets/images/woocommerce-icon.svg' ); ?>">
											</div>
											<div class="import-export-box-border">
												...
												<div class="import-export-button-section">
													...
													$wpssw_status = WPSSLC_Dependencies::check_plugin_install_and_activation_status($wpssw_plugin_slug, $wpssw_plugin_file);
													?>
										...
										<div class="import-export-plugin-info">
											<div class="import-export-box-border">
												<div class="import-export-pro-content">
													...
													<p class="text">Sync Elementor Form submissions with Google Sheets. Automate data management through real-time imports and exports.</p>
										
												<div class="import-export-button-section">
													<?php 
													$wpsse_plugin_slug = 'wpsyncsheets-elementor'; 
													$wpsse_plugin_file = 'wpsyncsheets-elementor/wpsyncsheets-lite-elementor.php';
													$wpsse_status = WPSSLC_Dependencies::check_plugin_install_and_activation_status($wpsse_plugin_slug, $wpsse_plugin_file);
													?>
											...
											<div class="import-export-feature-img">
												<img src="<?php echo esc_url( WPSSLC_URL . 'assets/images/Elementor-icon.svg' ); ?>">
											</div>
										</div>
									</div>
									...
									<div class="import-export-plugin-section">
										...
										<div class="import-export-plugin-info">
											<div class="import-export-feature-img">
												<img src="<?php echo esc_url( WPSSLC_URL . 'assets/images/GravityForms-icon.svg' ); ?>">
											</div>
											<div class="import-export-box-border">
												<div class="import-export-pro-content">
													<h1>WPSyncSheets For GravityForms</h1>
												</div>
												...
												<div class="import-export-button-section">
													...
													$wpssg_status = WPSSLC_Dependencies::check_plugin_install_and_activation_status($wpssg_plugin_slug, $wpssg_plugin_file);
													?>
										...
										<div class="import-export-plugin-info">
											<div class="import-export-box-border">
												...
												<div class="import-export-button-section">
													...
													$wpsswp_status = WPSSLC_Dependencies::check_plugin_install_and_activation_status($wpsswp_plugin_slug, $wpsswp_plugin_file);
													?>
											...
											<div class="import-export-feature-img">
												<img src="<?php echo esc_url( WPSSLC_URL . 'assets/images/WPForms-icon.svg' ); ?>">
											</div>
										</div>
									</div>
								...
								<div class="video-grid">
									<div class="video-item">
										<a href="#" target="_blank" class="play-icon" data-video-id="Z6-wzgpRlLE">
										    <img src="<?php echo esc_url( WPSSLC_URL . 'assets/images/WPSyncSheets-Intro.jpg' ); ?>" alt="">
										</a>
									</div>
									<div class="video-item">
										<a href="#" target="_blank" class="play-icon" data-video-id="EFLcBu-Jgp8">
										    <img src="<?php echo esc_url( WPSSLC_URL . 'assets/images/client-id-client-secret.jpg' ); ?>" alt="">
										</a>
									</div>
								</div>
								<div id="video-popup">
				...
				<div id="wpsslc-nav-googleapi" class="navtabcontent vertical-tabs">
					<div id="googleapi-settings" class="wpsslc-navtabcontent">
						<?php
						if ( ! current_user_can( 'edit_wpsyncsheets_contact_form_7_lite_main_settings' ) ) {
							?>
						<div class="wpsslc-top-subtext generalSetting-section">
							<div class="generalSetting-left">
								<h4><?php echo esc_html__( 'You do not have permission to access this page.', 'wpssc' ); ?></h4>
							</div>
						</div>
							<?php
						} else {
						...
						<form method="post" action="<?php echo esc_html( admin_url( 'admin.php?page=wpsyncsheets-contact-form-7' ) ); ?>">
							...
							<div id="universal-message-container">
								<div class="generalSetting-section">
									<div class="integrationtabform">
										<div class="options">
											<ul class="form-table">
												<li>
													<label for="client_id"> <?php echo esc_html__( 'Client ID', 'wpssc' ); ?> </label>
													<div class="forminp forminp-text">
														<input type="text" id="client_id" required name="client_id" value="<?php echo isset( $wpsslc_google_settings_value[0] ) ? esc_attr( $wpsslc_google_settings_value[0] ) : ''; ?>" size="80" class = "googlesettinginput" placeholder="Enter Client Id" 
															<?php
												...
												<li>
													<label for="client_secret"> <?php echo esc_html__( 'Client Secret Key', 'wpssc' ); ?> </label>
													<div class="forminp forminp-text">
														<input type="text" id="client_secret" required name="client_secret" value="<?php echo isset( $wpsslc_google_settings_value[1] ) ? esc_attr( $wpsslc_google_settings_value[1] ) : ''; ?>" size="80" class = "googlesettinginput" placeholder="Enter Client Secret" 
															<?php
												...
												if ( ! empty( $wpsslc_google_settings_value[0] ) && ! empty( $wpsslc_google_settings_value[1] ) ) {
														$wpsslc_token_value = $wpsslc_google_settings_value[2];
													?>
												<li>
													<label><?php echo esc_html__( 'Client Token', 'wpssc' ); ?></label>
													<?php
													if ( empty( $wpsslc_token_value ) && ! isset( $_GET['code'] ) ) {
														$wpsslc_auth_url = self::$instance_api->getClient();
														self::$instance_api->wpsslc_update_option( 'wpssc_share_enable', true );
														?>
														<div id="authbtn">
															<a href="<?php echo esc_url( $wpsslc_auth_url ); ?>" id="authlink" target="_blank" ><div class="wpsslc-button wpsslc-button-secondary"><?php echo esc_html__( 'Click here to generate an Authentication Token', 'wpssc' ); ?></div></a>
														</div>
														<?php
													}
													$wpsslc_code                  = '';
													...
													<div  id="authtext" 
													<?php
													if ( ! empty( $wpsslc_token_value ) || $wpsslc_code ) {
														echo 'class = "forminp forminp-text wpsslc-authtext" ';
													} else {
														echo 'class="forminp forminp-text"'; }
													?>
														><input type="text" name="client_token" value="<?php echo $wpsslc_token_value ? esc_attr( $wpsslc_token_value ) : esc_attr( $wpsslc_code ); ?>" size="80" placeholder="Please enter authentication code" id="client_token" class="googlesettinginput" 
															<?php
															if ( ! empty( $wpsslc_google_settings_value[2] ) ) {
																echo 'readonly';
															}
												...
												<?php } if ( ! empty( $wpsslc_token_value ) ) { ?>
												<li>
													<label></label>
													<div><input type="submit" name="revoke" id="revoke" value = "Revoke Token" class="wpsslc-button wpsslc-button-secondary"/></div>
												</li>
												<?php } ?>
											</ul>
										</div>
										...
										<div class="submit-section">
											<p class="submit">
												<input type="submit" name="submit" id="submit" class="wpsslc-button wpsslc-button-primary" value="Save">
												<?php
												if ( ! empty( $wpsslc_token_value ) || ! empty( $wpsslc_google_settings_value[0] ) || ! empty( $wpsslc_google_settings_value[1] ) ) {
													?>
														<input type="submit" name="reset_settings" id="reset_settings" value = "Reset" class="wpsslc-button wpsslc-button-primary reset_settings"/>
													<?php } ?>
											</p>
										</div>
									</div>
								...
								<div class="generalSetting-section copy-url-table">
									<ul>
										<li>
											<label><?php echo esc_html__( 'Authorized Domain : ', 'wpssc' ); ?></label>
											<div class="copy-url-text"><span id="authorized_domain"><?php echo esc_html( $site_url ); ?></span>
												<span class="copy-icon wpsslc-button" id="a_domain" onclick="wpsslcCopy('authorized_domain','a_domain');">
										...
										<li>
											<label><?php echo esc_html__( 'Authorised redirect URIs : ', 'wpssc' ); ?></label>
											<div class="copy-url-text">
												<span id="authorized_uri"><?php echo esc_html( admin_url( 'admin.php?page=wpsyncsheets-contact-form-7' ) ); ?></span>
												<span class="copy-icon wpsslc-button tooltip-click1" onclick="wpsslcCopy('authorized_uri','a_uri');" id="a_uri">
													<svg width="15" height="18" viewBox="0 0 15 18" fill="none" xmlns="http://www.w3.org/2000/svg">
													<path d="M2.16667 17.3334C1.70833 17.3334 1.31597 17.1702 0.989583 16.8438C0.663194 16.5174 0.5 16.1251 0.5 15.6667V4.00008H2.16667V15.6667H11.3333V17.3334H2.16667ZM5.5 14.0001C5.04167 14.0001 4.64931 13.8369 4.32292 13.5105C3.99653 13.1841 3.83333 12.7917 3.83333 12.3334V2.33341C3.83333 1.87508 3.99653 1.48272 4.32292 1.15633C4.64931 0.829942 5.04167 0.666748 5.5 0.666748H13C13.4583 0.666748 13.8507 0.829942 14.1771 1.15633C14.5035 1.48272 14.6667 1.87508 14.6667 2.33341V12.3334C14.6667 12.7917 14.5035 13.1841 14.1771 13.5105C13.8507 13.8369 13.4583 14.0001 13 14.0001H5.5ZM5.5 12.3334H13V2.33341H5.5V12.3334Z" fill="#383E46"/>
													</svg>
													<span class="tooltip-text">Copied</span>
				...
				<div id="wpsslc-nav-freevspro" class="navtabcontent vertical-tabs">
					<div class="wpsslc-navtabcontent">
								<!-- Header Section -->
								<div class="free-pro-header">
									...
									<div class="unlock-pro-button-section">
										<a class="underline" href="<?php echo WPSSLC_PRO_VERSION_BUY_URL;  ?>" target="_blank" rel="noreferrer">
											Upgrade Now
										</a>
									</div>
								</div>
								...
								<div class="pro-free-features">
									...
									<div class="feature-data">
										<p>Reorder Sheet Headers (Spreadsheet Columns)</p>
											<div class="feature-icon">
				...
				if ( ! empty( $wpsslc_error ) || ! empty( $wpsslc_error_general ) || ! \WPSSLC_Dependencies::wpsslc_is_contact_form_7_plugin_active() ) {
					$error_msg = '';
					if ( ! \WPSSLC_Dependencies::wpsslc_is_contact_form_7_plugin_active() ) {
						$error_msg = 'WPSyncSheets Lite For Contact Form 7 plugin requires <a href="' . esc_url( 'https://wordpress.org/plugins/contact-form-7/' ) . '" target= "_blank">Contact Form 7</a> plugin to be active!';
						?>
					<input type="hidden" id="error-message" value="<?php echo esc_attr( $error_msg ); ?>">
						<?php
					} else {
						if ( ! empty( $wpsslc_error_general ) ) {
							$error_msg = wp_strip_all_tags( $wpsslc_error_general );
						} else {
							$error_msg = wp_strip_all_tags( $wpsslc_error );
						}
						if ( $error_msg ) {
							?>
						<input type="hidden" id="error-message" value="<?php echo esc_attr( $error_msg ); ?>">
				...
				<script type="text/javascript">
					jQuery.noConflict();
					jQuery(".wpsslc-header-section .whatsNew-toggle, .wpsslc-header-section .close-block").click(function(){
						jQuery(".wpsslc-header-section .wpsslc-header-right").toggleClass('active');
						jQuery(".wpsslc-header-section .whatsNew-toggle").toggleClass('active');
					});
				</script>
                <div class="clear"></div>
			</div>
			...
			<!-- Unlock Pro Features -->
			<div class="wpsslc-tabs-main wpsslc-pro-main">
						...
						<div class="generalSetting-section">
							<div class="box-border">
								<div class="unlock-pro-content">
									...
									<p class="text">Sync Contact Form 7 data with Google Sheets in real-time — effortlessly and accurately.</p>
                                ...
                                <ul class="pro-features-content">
									<li>
										...
										<path d="M20 6 9 17l-5-5"></path></svg><span>Reorder Sheet Headers (Spreadsheet Columns)</span>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check text-brand-primary-600">
										<path d="M20 6 9 17l-5-5"></path></svg><span>Update Sheet Headers Name (Spreadsheet Columns)</span>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check text-brand-primary-600">
										<path d="M20 6 9 17l-5-5"></path></svg><span>Row Input Format Option</span>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check text-brand-primary-600">
								...
								<div class="unlock-pro-button-section">
									...
									<a class="no-underline" href="javascript:void(0)" onclick="wpsslcNavTabfreevspro(event, 'wpsslc-nav-freevspro')">
									    Free vs Pro
								    </a>
								</div>
							</div>
							...
							<div class="pro-feature-img">
								<img src="<?php echo esc_url( WPSSLC_URL . 'assets/css/images/pro.png' ); ?>" />
							</div>
						</div>	
					</div>
				</div>
	 ...
	 */
	public static function wpsslc_get_plugin_changelog( $plugin_path ) {
	...
	*/
	public static function wpsslc_install_and_activate_plugin() {
